...
#LDA算法
def Lda(film_user,film_topic,topic_user,film_user_count,user_film_score):
    ...
    for i in range(vars['iterations']):  #迭代次数，重复迭代就是为了收敛
        for f in range(len(film_user)): #遍历每个电影
            ...
            for u in range(len(film_user[f])): #遍历每个电影下的用户
                ...
                topic_user[t0][wid] = topic_user[t0][wid]-user_film_score[f][wid]
                ...
                topic_user[t1][wid] =topic_user[t1][wid]+user_film_score[f][wid]
                # if t0!=t1:
                #     print("=========="+str(i))
...
if __name__ == '__main__':
    ...
    print("开始训练模型")
    theta,phi = Lda(film_user,film_topic,topic_user,vars['user_count'],user_film_score)
